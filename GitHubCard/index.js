...
*/
axios.get('https://api.github.com/users/AJWD92').then(response => {
...
function GitCard(item) {
  ...
  UserProfile.src = item.html_url

  const UserFollowers = document.createElement('p')
  UserFollowers.textContent = item.followers

  const UserFollowing = document.createElement('p')
  UserFollowing.textContent = item.following

  const UserBio = document.createElement('p')
  UserBio.textContent = item.bio

  card.appendChild(InfoCard);
  card.appendChild(AvaImg);
